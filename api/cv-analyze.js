...
Analyse the CV against the job requirements and return ONLY a valid JSON object — no markdown, no code fences:
{
  ...
  "strengths": ["<2-4 things in her CV that are already well-aligned to this role's requirements — be specific>"],
  "gaps": ["<2-4 things missing from or under-represented in her CV relative to what this role needs>"],
  "tweaks": ["<3-5 specific, actionable CV edits — e.g. 'In your UPS role, move the Power BI bullet to the top and quantify: X dashboards built for Y stakeholders', 'Add a brief mention of A/B testing results to your Amazon internship', 'Your profile summary should open with go-to-market strategy, not logistics'>"],
  ...
  try {
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: 'user',
          content: `TARGET ROLE: ${jobTitle} at ${company || 'company not specified'}\n\nJOB DESCRIPTION:\n${jobDescription.slice(0, 5000)}\n\n---\n\nMY CV:\n${cvText.slice(0, 8000)}`,
        }],
      }),
    });
